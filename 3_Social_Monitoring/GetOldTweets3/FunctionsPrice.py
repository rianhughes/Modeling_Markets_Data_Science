...
#filename_k = 'C_Price_Data/Kraken_BTCUSD_d.csv'  
#filename_b = 'C_Price_Data/Bitfinex_BTCUSD_d.csv'
...
def GetPrice_on_Date(filename_price, ref_year, ref_month, ref_day):
    """
    GetPrice for a Given Date
    """
    df1=pd.read_csv(filename_price)
    if(ref_day<10):
        ref_date1 = str(ref_year)+'-0'+str(ref_month)+'-0'+str(ref_day)
    else:
        ref_date1 = str(ref_year)+'-0'+str(ref_month)+'-'+str(ref_day)
    return float(df1[df1['Date']==ref_date1]['Open'])
